...
function siteorigin_share_render( $settings = array() ) {
	...
	<ul class="share-buttons">
		<li class="network facebook" style="width:<?php echo intval( $settings['width'] ) ?>px">
		...
		<li class="network twitter" style="width:<?php echo intval( $settings['width'] ) ?>px">
			<?php
			$related = array();
			$related[ ] = $settings['twitter'];
			$twitter_url = add_query_arg( array(
				'url' => get_permalink(),
				'via' => atomlabs_setting( 'social_twitter' ),
				'text' => get_the_title(),
				'related' => implode( ',', $related )
			), 'http' . (is_ssl() ? 's' : '' ) . '://platform.twitter.com/widgets/tweet_button.html' );

			?>
			<iframe allowtransparency="true" frameborder="0" scrolling="no" src="<?php echo esc_attr( $twitter_url ) ?>" style="width:<?php echo intval( $settings['width'] ) ?>px; height:20px;"></iframe>
		</li>

		<li class="network plusone" style="width:<?php echo intval( $settings['width'] ) ?>px">
