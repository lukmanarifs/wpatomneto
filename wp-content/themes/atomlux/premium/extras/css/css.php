...
function siteorigin_custom_css_admin_menu() {
	add_theme_page( __( 'Custom CSS', 'atom' ), __( 'Custom CSS', 'atom' ), 'edit_theme_options', 'siteorigin_custom_css', 'siteorigin_custom_css_page' );
...
function siteorigin_custom_css_help() {
	...
	$screen->add_help_tab( array(
		'id' => 'custom-css',
		'title' => __( 'Custom CSS', 'atom' ),
		'content' => '<p>'
			. sprintf( __( "%s adds any custom CSS you enter here into your site's header. ", 'atom' ), ucfirst( $theme ) )
			. __( "These changes will persist across updates so it's best to make all your changes here. ", 'atom' )
			. sprintf( __( "Contact <a href='%s' target='_blank'>SiteOrigin Support</a> for help with making edits. ", 'atom' ), 'http://siteorigin.com/support/contact/' )
			. '</p>'
	) );
}
	...
	<div class="wrap">
		...
		<form action="<?php echo esc_url( add_query_arg( null, null ) ) ?>" method="POST" style="margin-top:25px">
			...
			<p class="description">
				<?php
				$theme = basename( get_template_directory() );
				printf( __( 'Changes apply to %s and its child themes', 'atom' ), ucfirst( $theme ) );
				?>
			</p>
	
			<p class="submit">
				<input type="submit" name="siteorigin_custom_css_save" class="button-primary" value="<?php esc_attr_e( 'Save CSS', 'atom' ); ?>" />
			</p>
