...
function siteorigin_mobilenav_enqueue_scripts() {
	wp_enqueue_script( 'atom-mobilenav', get_template_directory_uri() . '/premium/extras/mobilenav/js/mobilenav.min.js', array( 'jquery' ), NULL );

	$text = array(
		'navigate' => __( 'Menu', 'atom' ),
		'back' => __( 'Back', 'atom' ),
		'close' => __( 'Close', 'atom' ),
	);
	$text = apply_filters('siteorigin_mobilenav_text', $text);

	wp_localize_script( 'atom-mobilenav', 'mobileNav', array(
		'search' => array( 'url' => get_site_url(), 'placeholder' => __( 'Search', 'atom' ) ),
		'text' => $text,
		'nextIconUrl' => get_template_directory_uri().'/premium/extras/mobilenav/images/next.png',
	) );
	wp_enqueue_style( 'atom-mobilenav', get_template_directory_uri() . '/premium/extras/mobilenav/css/mobilenav.css', array(), NULL );
}
