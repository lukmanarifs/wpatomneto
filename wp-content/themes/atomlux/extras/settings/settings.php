 ...
 */
function atomlabs_settings_init( $theme_name = null ) {
	...
	$GLOBALS['atomlabs_settings_theme_name'] = $theme_name;
	$GLOBALS['atomlabs_settings_name'] = $theme_name . '_theme_settings';
	$GLOBALS['atomlabs_settings_defaults'] = apply_filters( 'siteorigin_theme_default_settings', array() );
	...
	$GLOBALS['atomlabs_settings'] = wp_parse_args( $settings, $GLOBALS['atomlabs_settings_defaults'] );

	// Register all the actions for the settings page
	add_action( 'admin_menu', 'atomlabs_settings_admin_menu' );
	add_action( 'admin_init', 'atomlabs_settings_admin_init', 8 );
	add_action( 'siteorigin_adminbar', 'atomlabs_settings_adminbar' );

	add_action( 'admin_enqueue_scripts', 'atomlabs_settings_enqueue_scripts' );
	add_action( 'wp_enqueue_scripts', 'atomlabs_settings_enqueue_front_scripts' );
}
add_action('after_setup_theme', 'atomlabs_settings_init', 5);
 ...
 * @action admin_init
 */
function atomlabs_settings_admin_init() {
	register_setting( 'theme_settings', $GLOBALS['atomlabs_settings_name'], 'atomlabs_settings_validate' );
 ...
 * @action admin_menu
 */
function atomlabs_settings_admin_menu() {
	$theme = wp_get_theme();
	$page = add_theme_page(
		sprintf(__( '%s Theme Settings', 'atom' ), $theme->get('Name') ),
		sprintf(__( 'Theme Settings', 'atom' ), $theme->get('Name') ),
		'edit_theme_options',
		'theme_settings_page',
		'atomlabs_settings_render'
	);

	add_action( 'load-' . $page, 'atomlabs_settings_theme_help' );
}

/**
 * Render the theme settings page
 */
function atomlabs_settings_render() {
	if( version_compare( get_bloginfo('version'), '3.4', '<' ) ) {
		?><div class="wrap"><div id="setting-error-settings_updated" class="updated settings-error"><p><strong><?php _e('Please update to the latest version of WordPress to use theme settings.', 'atom') ?></strong></p></div></div><?php
		return;
	}
 ...
 * @param $prefix
 */
function atomlabs_settings_enqueue_scripts( $prefix ) {
 ...
 */
function atomlabs_settings_enqueue_front_scripts(){
	if( current_user_can('manage_options') && atomlabs_setting('general_hover_edit', true) ) {
 ...
 * @return object|null
 */
function atomlabs_settings_adminbar( $bar ) {
	$screen = get_current_screen();
	if ( $screen->id == 'appearance_page_theme_settings_page' ) {
		$bar = (object)array( 'id' => $GLOBALS['atomlabs_settings_name'], 'message' => array( 'extras/settings/message' ) );
	}
 ...
 * @param $name
 */
function atomlabs_settings_add_section( $id, $name ) {
 ...
 * @param array $args
 */
function atomlabs_settings_add_field( $section, $id, $type, $title = null, $args = array() ) {
	...
	if ( isset( $wp_settings_fields[ 'theme_settings' ][ $section ][ $id ] ) ) {
		if ( isset( $wp_settings_fields[ 'theme_settings' ][ $section ][ $id ][ 'args' ][ 'type' ] ) && $wp_settings_fields[ 'theme_settings' ][ $section ][ $id ][ 'args' ][ 'type' ] == 'teaser' ) {
			...
			$wp_settings_fields[ 'theme_settings' ][ $section ][ $id ] = array(
				'id' => $id,
				'title' => $title,
				'callback' => 'atomlabs_settings_field',
				'args' => $args
			);
		}
		else return;
	...
	add_settings_field( $id, $title, 'atomlabs_settings_field', 'theme_settings', $section, $args );
}
 ...
 * @param array $args
 */
function atomlabs_settings_add_teaser( $section, $id, $name, $args = array() ) {
	global $wp_settings_fields;
	...
	add_settings_field( $id, $name, 'atomlabs_settings_field', 'theme_settings', $section, $args );
}
 ...
 * @return mixed
 */
function atomlabs_setting( $name , $default = null) {
	$value = null;
	
	if ( !is_null( $default ) && ( !is_bool( $GLOBALS[ 'atomlabs_settings' ][ $name ] ) && empty( $GLOBALS[ 'atomlabs_settings' ][ $name ] ) ) ) {
		return apply_filters('atomlabs_setting_'.$name, $default);
	}
	
	if ( !isset( $GLOBALS[ 'atomlabs_settings' ][ $name ] ) ) $value = null;
	else $value = $GLOBALS['atomlabs_settings'][ $name ];

	return apply_filters('atomlabs_setting_'.$name, $value);
}

/**
 * Adds the necessary classes to make a field editable
 */
function atomlabs_setting_editable($field){
	if( current_user_can('manage_options') && atomlabs_setting('general_hover_edit', true) ) {
 ...
 */
function atomlabs_setting_editable_option_field(){
	atomlabs_settings_add_field( 'general', 'hover_edit', 'checkbox', __( 'Display Hover Edit Icon', 'atom' ), array(
		'description' => __( 'Display a small icon that makes quickly editing fields easy. This is only shown to admin users.', 'atom' ),
	) );
}
add_action('admin_init', 'atomlabs_setting_editable_option_field', 100);

function atomlabs_setting_editable_option_default($defaults){
	$defaults['general_hover_edit'] = true;
	return $defaults;
}
add_filter('siteorigin_theme_default_settings', 'atomlabs_setting_editable_option_default');
 ...
 * @param $args
 */
function atomlabs_settings_field( $args ) {
	$field_name = $GLOBALS['atomlabs_settings_name'] . '[' . $args['section'] . '_' . $args['field'] . ']';
	$field_id = $args['section'] . '_' . $args['field'];
	$current = isset( $GLOBALS['atomlabs_settings'][ $field_id ] ) ? $GLOBALS['atomlabs_settings'][ $field_id ] : null;

	switch ( $args['type'] ) {
		case 'checkbox' :
			?>
			<input id="<?php echo esc_attr( $field_id ) ?>" name="<?php echo esc_attr( $field_name ) ?>" type="checkbox" <?php checked( $current ) ?> />
			<label for="<?php echo esc_attr( $field_id ) ?>"><?php echo esc_attr( !empty( $args['label'] ) ? $args['label'] : __( 'Enabled', 'atom' ) ) ?></label>
			<?php
			break;
		case 'text' :
		case 'number' :
		...
		case 'media':
			if(version_compare(get_bloginfo('version'), '3.5', '<')){
				printf(__('You need to <a href="%s">upgrade</a> to WordPress 3.5 to use media fields', 'atom'), admin_url('update-core.php'));
				break;
			}
			...
			$choose_title = empty($args['choose']) ? __('Choose Media', 'atom') : $args['choose'];
			$update_button = empty($args['update']) ? __('Set Media', 'atom') : $args['update'];
			...
			?>
				<div class="media-field-wrapper">
					...
					<a href="#" class="media-remove-button"><?php _e('Remove', 'atom') ?></a>
				</div>
		...
		case 'teaser' :
			...
			<a class="premium-teaser siteorigin-premium-teaser" href="<?php echo admin_url( 'themes.php?page=premium_upgrade' ) ?>" target="_blank">
				<em></em>
				<?php printf( __( 'Only available in <strong>%s Premium</strong> - <strong class="upgrade">Upgrade Now</strong>', 'atom' ), ucfirst($theme) ) ?>
		...
		case 'gallery' :
			...
			<input
				...
				type="text"
				value="<?php echo esc_attr( $current ) ?>" />
			<a href="#" class="so-settings-gallery-edit"><?php _e('Select Images', 'atom') ?></a>
			<?php
			break;
		...
		case 'pages' :
			...
			?>
			<select id="<?php echo esc_attr( $field_id ) ?>" name="<?php echo esc_attr( $field_name ) ?>">
				<option value="0"><?php _e('None', 'atom') ?></option>
 ...
 * @return array
 */
function atomlabs_settings_validate( $values ) {
	global $wp_settings_fields;
	...
	foreach ( $wp_settings_fields['theme_settings'] as $section_id => $fields ) {
		foreach ( $fields as $field_id => $field ) {
			...
			switch($field['args']['type']){
				...
				case 'number' :
					// Only allow integers
					$values[ $name ] = isset( $values[ $name ] ) ? intval( $values[ $name ] ) : $GLOBALS['atomlabs_settings_defaults'][ $name ];
					break;
	...
	if ( $changed ) {
		do_action( 'atomlabs_settings_changed' );
		set_theme_mod( 'atomlabs_settings_changed', true );
	}
 ...
 */
function atomlabs_settings_change_message() {
	if ( get_theme_mod( 'atomlabs_settings_changed' ) ) {
		remove_theme_mod( 'atomlabs_settings_changed' );

		?>
		<div id="setting-updated" class="updated">
			<p>
				<strong><?php _e( 'Settings saved.', 'atom' ) ?></strong>
			</p>
		</div>
		<?php
		 ...
		 */
		do_action( 'atomlabs_settings_changed_message' );
	}
}

function atomlabs_settings_theme_help(){
	$screen = get_current_screen();
	$theme_name = basename( get_template_directory() );
	
	$text = sprintf(
		__( "Read %s's <a href='%s'>theme documentation</a> for help with these settings.", 'atom' ),
		ucfirst($theme_name),
		'http://siteorigin.com/theme/'.$theme_name.'/?action=docs'
	); 
	
	$screen->add_help_tab( array(
		'id' => 'atomlabs_settings_help_tab',
		'title' => __( 'Settings Help', 'atom' ),
		'content' => '<p>' . $text . '</p>',
	) );
}
 ...
 */
function atomlabs_settings_template_part_names($parts, $part_name){
	$return = array();
...
function atomlabs_settings_media_view_strings($strings, $post){
	...
	$strings['insertIntoPost'] = __('Set Media File', 'atom');
	
	return $strings;
}
add_filter('media_view_strings', 'atomlabs_settings_media_view_strings', 10, 2);
