...
class AtomCoreMegaMenu {

	function AtomCoreMegaMenu(){
		
		if (is_admin()){
			// add style, js when edit menu
			add_action( 'admin_menu' , array( $this , 'atomcore_admin_menu'));
			// add custom menu options when edit menu
			add_filter( 'wp_edit_nav_menu_walker', array($this, 'atomcore_wp_edit_nav_menu_walker'), 10, 2 );
			// update custom menu option after edit
			add_action( 'wp_update_nav_menu_item', array( $this , 'atomcore_nav_menu_item_custom_update'),10, 3);
		}
		// get custom field value
		add_filter( 'wp_setup_nav_menu_item',array( $this , 'atomcore_nav_menu_item_custom'));
	...
	function atomcore_admin_menu(){
		...
		if($pagenow == "nav-menus.php"){
			//style
			wp_enqueue_style( 'fontawesome', $dir . '/assets/fontawesome/css/font-awesome.min.css' , array() , $ver );
			wp_enqueue_style( 'atomcore-megamenu', $dir . FRAMEWORK_PATH . '/style/atomcore-megamenu.css' , array() , $ver );
			if ( is_rtl() ) {
				wp_enqueue_style( 'atomcore_rtl', $dir . FRAMEWORK_PATH . '/style/rtl-megamenu.css' , array() , $ver );
			}
			//scripts
			wp_enqueue_script( 'jquery');
			wp_enqueue_script( 'atomcore-megamenu', $dir . FRAMEWORK_PATH . '/scripts/atomcore-megamenu.js' , array('jquery'), $ver, true);
		}
	}
	 ...
	 */
	function atomcore_wp_edit_nav_menu_walker($class, $menu_id){
		return 'AtomCore_Walker_Nav_Menu_Edit';
	}
	
	/**
	 * out custom field value
	 */
	function atomcore_nav_menu_item_custom($menu_item){
		$post_fields = array(
	 ...
	 */
	function atomcore_nav_menu_item_custom_update($menu_id, $menu_item_db_id, $args){
 ...
 */
class AtomCore_Walker extends Walker {
	/**
 ...
 */
class AtomCore_Walker_Nav_Menu_Edit extends Walker_Nav_Menu {
	/**
	 * Starts the list before the elements are added.
	 *
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! empty( $item->_invalid ) ) {
			...
			$title = sprintf( __( '%s (Invalid)' ,'atom'), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( __('%s (Pending)','atom'), $item->title );
		}
		...
		<li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes ); ?>">
			<dl class="menu-item-bar">
				<dt class="menu-item-handle">
                	<span class="atomcore-megamenu-handle"><?php _e('Mega Menu','atom'); ?></span>
                    <span class="atomcore-megamenu-handle-sub"><?php _e('Sub Menu','atom'); ?></span>
					<span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo $submenu_text; ?>><?php _e( 'sub item' ,'atom'); ?></span></span>
					<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up','atom'); ?>">&#8593;</abbr></a>
							|
							<a href="<?php
								echo wp_nonce_url(
									add_query_arg(
							...
							?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down','atom'); ?>">&#8595;</abbr></a>
						</span>
						<a class="item-edit" id="edit-<?php echo $item_id; ?>" title="<?php esc_attr_e('Edit Menu Item','atom'); ?>" href="<?php
							echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
						?>"><?php _e( 'Edit Menu Item','atom' ); ?></a>
					</span>
				</dt>
			</dl>
			<div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
            	<?php // custom field for mega menu  start?>
            	<div class="atomcore-megamenu">
                	<h4 class="atomcore-megamenu-title"><?php _e( 'Mega Menu Setting','atom' ); ?></h4>
                	<p class="atomcore-megamenu-enable">
                    	<label for="edit-menu-item-megamenu-<?php echo $item_id; ?>">
                            <input type="checkbox" class="atomcore-megamenu-enable-checkbox" id="edit-menu-item-megamenu-<?php echo $item_id; ?>" value="enabled" name="menu-item-megamenu[<?php echo $item_id; ?>]" <?php checked( $item->megamenu, 'enabled' ); ?>/>
                            <?php _e( 'Mega Menu Enabled','atom' ); ?>
                        </label>
                    </p>
                    
                    <p class="atomcore-megamenu-direction">
                        ...
                        <label for="edit-menu-item-megadirection-<?php echo $item_id; ?>-2">
                            ...
                            <?php _e( 'Vertical','atom' ); ?>
                        </label>
                    </p>
                    
                    <p class="atomcore-megamenu-wideenable">
                    	<label for="edit-menu-item-megawide-<?php echo $item_id; ?>">
                            <input type="checkbox" class="atomcore-megamenu-megawide-checkbox" id="edit-menu-item-megawide-<?php echo $item_id; ?>" value="enabled" name="menu-item-megawide[<?php echo $item_id; ?>]" <?php checked( $item->megawide, 'enabled' ); ?>/>
                            <?php _e( 'Sub Menu Full Width Show Enabled','atom' ); ?>
                        </label>
                    </p>
                    
                   	<p class="atomcore-megamenu-megaimgenable">
                    	<label for="edit-menu-item-megaimgenable-<?php echo $item_id; ?>">
                            <input type="checkbox" class="atomcore-megamenu-megaimgenable-checkbox" id="edit-menu-item-megaimgenable-<?php echo $item_id; ?>" value="enabled" name="menu-item-megaimgenable[<?php echo $item_id; ?>]" <?php checked( $item->megaimgenable, 'enabled' ); ?>/>
                            <?php _e( 'Only Show Image Enabled','atom' ); ?>
                            <span class="description"><?php _e('The enabled need you input Image Url option and use menu default menu link.Description can been showed when you already input.','atom'); ?></span>
                        </label> 
                    </p>
                    
                    <p class="atomcore-megamenu-megawidget">
                        <label for="edit-menu-item-megawidget-<?php echo $item_id; ?>">
                            <?php _e( 'Widget Name (optional)','atom' ); ?><br />
                    ...
                    <p class="atomcore-megamenu-megashorttitle">
                        <label for="edit-menu-item-megashorttitle-<?php echo $item_id; ?>">
                            <?php _e( 'Short Title (optional)','atom' ); ?><br />
                    ...
                    <p class="atomcore-megamenu-megaimage">
                        <label for="edit-menu-item-megaimage-<?php echo $item_id; ?>">
                            <?php _e( 'Image Url (optional)','atom' ); ?><br />
                     ...
                     <p class="atomcore-megamenu-megaimglink">
                        <label for="edit-menu-item-megaimglink-<?php echo $item_id; ?>">
                            <?php _e( 'Image Link (optional)','atom' ); ?><br />
                    ...
                    <p class="atomcore-megamenu-megadescription">
                        <label for="edit-menu-item-megadescription-<?php echo $item_id; ?>">
                            <?php _e( 'Description (optional)','atom' ); ?><br />
				...
				<p class="description description-thin">
					<label for="edit-menu-item-title-<?php echo $item_id; ?>">
						<?php _e( 'Navigation Label' ,'atom'); ?><br />
						<input type="text" id="edit-menu-item-title-<?php echo $item_id; ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
					</label>
				</p>
				<p class="description description-thin">
					<label for="edit-menu-item-attr-title-<?php echo $item_id; ?>">
						<?php _e( 'Title Attribute' ,'atom'); ?><br />
				...
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo $item_id; ?>">
						...
						<?php _e( 'Open link in a new window/tab' ,'atom'); ?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo $item_id; ?>">
						<?php _e( 'CSS Classes (optional)' ,'atom'); ?><br />
						<input type="text" id="edit-menu-item-classes-<?php echo $item_id; ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo $item_id; ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
					</label>
				</p>
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo $item_id; ?>">
						<?php _e( 'Link Relationship (XFN)' ,'atom'); ?><br />
						<input type="text" id="edit-menu-item-xfn-<?php echo $item_id; ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
					</label>
				</p>
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo $item_id; ?>">
						<?php _e( 'Description' ,'atom'); ?><br />
						<textarea id="edit-menu-item-description-<?php echo $item_id; ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo $item_id; ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
						<span class="description"><?php _e('The description will be displayed in the menu if the current theme supports it.','atom'); ?></span>
					</label>
				</p>

				<p class="field-move hide-if-no-js description description-wide">
					<label>
						<span><?php _e( 'Move' ,'atom'); ?></span>
						<a href="#" class="menus-move-up"><?php _e( 'Up one' ,'atom'); ?></a>
						<a href="#" class="menus-move-down"><?php _e( 'Down one' ,'atom'); ?></a>
						<a href="#" class="menus-move-left"></a>
						<a href="#" class="menus-move-right"></a>
						<a href="#" class="menus-move-top"><?php _e( 'To the top' ,'atom'); ?></a>
					</label>
				</p>
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( __('Original: %s','atom'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
					...
					); ?>"><?php _e( 'Remove' ,'atom'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo $item_id; ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
						?>#menu-item-settings-<?php echo $item_id; ?>"><?php _e('Cancel','atom'); ?></a>
				</div>
