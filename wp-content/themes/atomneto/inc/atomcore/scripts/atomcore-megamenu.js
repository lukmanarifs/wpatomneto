...
jQuery(document).ready(function($) {
	...
	/* create atomcore object */
	var atomcore = new Object({
		init:function(){
			...
			function checkMenuChanged(){
				$('#menu-to-edit li').each(function() {
					if($(this).hasClass('atomcore-had-add-event')){
						return;
					}else{
			...
			function addMenuChangeEvent(element){
				$(element).addClass('atomcore-had-add-event');
				var menu = $(element);
				
				/* enabled mega menu checkbox */
				menu.find('.atomcore-megamenu-enable-checkbox').click(function() {
				...
				menu.find('.atomcore-megamenu-direction input').click(function() {
					refreshMegaMenu();
				});
			...
			function refreshMegaMenu(){
				...
				for(var i=0; i<items.length; i++){
					if($(items[i]).hasClass('menu-item-depth-0')){
						if($(items[i]).find('.atomcore-megamenu-enable-checkbox').attr('checked') == "checked"){
							main_menu = true;
						...
						if(megamenu_enabled){
							...
							if($($(current_main).find('.atomcore-megamenu-direction input').get(0)).attr('checked') == "checked"){
