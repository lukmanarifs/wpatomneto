...
<form id="yith-wcwl-form" action="<?php echo esc_url( $yith_wcwl->get_wishlist_url() ) ?>" method="post">
    ...
    <table class="shop_table cart wishlist_table" cellspacing="0">
        ...
        <tr>
            ...
            <th class="product-name"><span class="nobr"><?php _e( 'Product Name', 'atom' ) ?></span></th>
            <?php if( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : ?><th class="product-price"><span class="nobr"><?php _e( 'Unit Price', 'atom' ) ?></span></th><?php endif ?>
            <?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?><th><span class="nobr"><?php _e( 'Stock Status', 'atom' ) ?></span></th><?php endif ?>
        ...
        if( count( $wishlist ) > 0 ) :
            foreach( $wishlist as $values ) :
                ...
                if( $product_obj !== false && $product_obj->exists() ) : ?>
                    <tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
                        <td class="product-remove"><div><a href="javascript:void(0)" onclick="remove_item_from_wishlist( '<?php echo esc_url( $yith_wcwl->get_remove_url( $values['ID'] ) )?>', 'yith-wcwl-row-<?php echo $values['ID'] ?>');" class="remove" title="<?php _e( 'Remove this product', 'atom' ) ?>">&times;</a></td>
                        ...
                        <?php if( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : ?>
                            <td class="product-price">
                                ...
                                if( $product_obj->price != '0' ) {
                                    ...
                                    echo apply_filters( 'yith_free_text', __( 'Free!', 'atom' ) );
                                }
                                ?>
                            </td>
                        <?php endif ?>
                        <?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?>
                            <td class="product-stock-status">
                                ...
                                if( $stock_status == 'out-of-stock' ) {
                                    $stock_status = "Out";
                                    echo '<span class="wishlist-out-of-stock">' . __( 'Out of Stock', 'atom' ) . '</span>';
                                } else {
                                    $stock_status = "In";
                                    echo '<span class="wishlist-in-stock">' . __( 'In Stock', 'atom' ) . '</span>';
                                }
                                ?>
                            </td>
                        <?php endif ?>
            ...
            <tr>
                <td colspan="6" class="wishlist-empty"><?php _e( 'No products were added to the wishlist', 'atom' ) ?></td>
            </tr>
        <?php
        endif;
	...
	if(atom_get_options_key('woocommerce-share-enable') == "on"){ ?> 
	<div class="woocommerce-share">
	<?php if(intval(atom_get_options_key('woocommerce-share-type')) == 0) {
		echo do_shortcode('[share link="'.esc_url( get_permalink() ).'" title="'.esc_attr( get_the_title() ).'"]');
	 } else { ?>
	<?php echo  atom_get_options_key('woocommerce-share-content'); ?>
	<?php } ?>
	</div>
<?php }
