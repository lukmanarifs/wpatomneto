...
if(atom_get_post_meta_key( 'title-show', $p_ID) != "off"){
?>
	...
	<header id="site-content-header">
			<div class="container">
				<div class="page-title">
					<?php 
						$title 	= 	atom_get_post_meta_key('title-content', $p_ID);
						$desc	=	atom_get_post_meta_key('title-desc', $p_ID);
						$enable_breadcrumb	=	atom_get_post_meta_key('title-breadcrumb', $p_ID);
						if($title == ''){ 
							echo '<h1 class="title">';
							if(is_single()){
								echo get_the_title();
...
if ( $shop_page_id && strstr( $permalinks['product_base'], '<span class="breadcrumb-right">/</span>' . $shop_page->post_name ) && get_option( 'page_on_front' ) !== $shop_page_id ) {
...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == woocommerce_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( ! empty( $home ) ) {
		echo $before . '<a class="home" href="' . apply_filters( 'woocommerce_breadcrumb_home_url', home_url() ) . '"><i class="fa fa-home"></i></a>' . $after . $delimiter;
	}
